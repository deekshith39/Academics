...
def NumberofMisplacedTiles(temp):
    count = 0
    for i in range(1,9):
        currStateIndex = index(temp, i)
        goalStateIndex = index(goal, i)
        if currStateIndex != goalStateIndex:
            count += 1
    return count
...
def solve(visited,limit,src):
    ...
    for action in possible_actions:
        count = NumberofMisplacedTiles(action)
        if action not in visited and count < min :
            min = count
            new_move = action
    print("Move : ",limit + 1)
